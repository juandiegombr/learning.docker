...
start: ## Run the application
	docker compose -f ./docker-compose.yml -p docker-demo-all up --build

start-web: ## Run the web application
	docker compose -f web/docker-compose.yml -p docker-demo-web up --build

start-web-hmr: ## Run the web application
	docker-sync start -c web/docker/dev/docker-sync.yml
	docker compose -f web/docker/dev/docker-compose.yml -p docker-demo-web run --rm -p 3000:3000 node
...
down-api: ## Destroy the api application
	docker compose -f api/docker-compose.yml -p docker-demo-api down
